...
def function_call(filename: str, content: str, filetype: str = "py"):
    ...
    return f"l'utilisateur a maintenant acces a un bouton pour télécharé sont fichier tu peux lui dire vous pouver cliquer sur le bouton download pour le téléchargé ci dessus mime="application/json" if filetype == "json" else f"text/{filetype}",
        key=f"download_{filename}""
